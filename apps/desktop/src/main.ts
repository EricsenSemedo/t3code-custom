...
import {
  ...
  resolveLegacyDesktopUserDataDirName,
...
const BASE_DIR =
  ...
  Path.join(OS.homedir(), resolveDefaultT3HomeDirName(isDevelopment));
const STATE_DIR = Path.join(BASE_DIR, "userdata");
const APP_DISPLAY_NAME = resolveAppDisplayName(isDevelopment);
const APP_USER_MODEL_ID = DESKTOP_APP_ID;
const LINUX_DESKTOP_ENTRY_NAME = resolveLinuxDesktopEntryName(isDevelopment);
const LINUX_WM_CLASS = resolveLinuxWmClass(isDevelopment);
const USER_DATA_DIR_NAME = resolveDesktopUserDataDirName(isDevelopment);
const LEGACY_USER_DATA_DIR_NAME = resolveLegacyDesktopUserDataDirName(isDevelopment);
 ...
 * parentheses (e.g. `~/.config/T3 Code Custom (Alpha)` on Linux). This is
