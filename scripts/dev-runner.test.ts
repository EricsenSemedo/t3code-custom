  ...
  describe("createDevRunnerEnv", () => {
    it.effect("defaults T3CODE_HOME to ~/.t3-custom when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.T3CODE_HOME, resolve(homedir(), ".t3-custom"));
      }),
    );
    ...
    it.effect("uses ~/.t3-custom-dev by default for dev:desktop", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: {},
        ...
        assert.equal(env.T3CODE_HOME, resolve(homedir(), ".t3-custom-dev"));
      }),
    );
  });
